...
export async function POST(request: NextRequest) {
  try {
    const { application_id, question_id, current_draft, instruction } = await request.json();
    if (!application_id || !question_id || !current_draft || !instruction) {
    ...
    const systemPrompt = [
      ...
      'You are editing a draft. Apply the instruction to refine it — do not restart from scratch. Output only the revised response, no preamble.',
    ]
      .filter(Boolean)
      .join('\n\n');
